...
- @sec-fCPA contains the code for the fPCA analysis, which is described in §3.4 of the manuscript
...
- @sec-tangent examines the possibility  that speakers with predominantly high uPC1 scores and predominantly low uPC1 scores have different monophthong spaces, and
that the different trajectory we see between high and low uPC1 tokens are actually a reflection of these differences (Footnote 1 of the manuscript)
...
int_filtered_ua <- read.csv(here("public", "Data", "Filtered", "Int", "ua.csv"))
...
results_extreme_all <- rbind(earesults, oaresults, uaresults, iaresults)
```
...
# Are the different trajectories for high and low uPC1 tokens a product of different monophthong spaces? (Manuscript footnote 1) {#sec-tangent}
As mentioned in Footnote 1 of the manuscript, it  could be possible that speakers with predominantly high uPC1 scores and predominantly low uPC1 scores have different monophthong spaces, and
...
```
No signficant effect of mora position by itself. Indicates  that effect lies in 
interactions. 
...
c) Is there a signficant overall effect of complex?
```{r}
...
d) Is there an overall effect of mora position?
```{r}
...
e) Investigate interaction with category further by conducting an EMM test
```{r}
